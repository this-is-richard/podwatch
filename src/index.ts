...
// Function to find and set up AWS CLI path
const setupAWSPath = (): void => {
  ...
  const commonPaths = [
    ...
    path.join(process.env.PROGRAMFILES || "", "Amazon", "AWSCLI", "bin"),
  ];

  // Get current PATH
  const currentPath = process.env.PATH || "";
  const pathSeparator = process.platform === "win32" ? ";" : ":";
  const awsExecutable = process.platform === "win32" ? "aws.exe" : "aws";

  // Find AWS CLI in common paths
  const foundPaths: string[] = [];

  for (const commonPath of commonPaths) {
    try {
      const awsPath = path.join(commonPath, awsExecutable);
      if (fs.existsSync(awsPath)) {
        foundPaths.push(commonPath);
      }
    } catch (error) {
  ...
  // Add found paths to PATH if not already present
  if (foundPaths.length > 0) {
    console.log("Found AWS CLI in paths:", foundPaths);
    const newPaths = foundPaths.filter(
      (foundPath) => !currentPath.split(pathSeparator).includes(foundPath)
    );

    if (newPaths.length > 0) {
      const updatedPath = [currentPath, ...newPaths].join(pathSeparator);
      process.env.PATH = updatedPath;
      console.log("Updated PATH to include AWS CLI paths:", newPaths);
    } else {
...
ipcMain.handle(
  "stream-logs",
  async (event, podName: string, contextName: string, namespace: string) => {
    try {
